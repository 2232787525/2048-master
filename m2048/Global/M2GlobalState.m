...
@property (nonatomic, readwrite) NSInteger dimension;//维度  4*4的格
@property (nonatomic, readwrite) NSInteger winningLevel;
@property (nonatomic, readwrite) NSInteger tileSize;    //小格子的宽度 不连线条宽度
@property (nonatomic, readwrite) NSInteger borderWidth; //格子线条的宽度
@property (nonatomic, readwrite) NSInteger cornerRadius;//所有的圆角角度
@property (nonatomic, readwrite) NSInteger horizontalOffset;
@property (nonatomic, readwrite) NSInteger verticalOffset;
@property (nonatomic, readwrite) NSTimeInterval animationDuration;//动画间隔时长
@property (nonatomic, readwrite) M2GameType gameType; //type = 0; 2*2 1：3*3*3 3个合一个 2：fibonacci 斐波那契数列 f(n)= f(n-1)+f(n-2); f(0)=0;f(1)=1
...
- (void)setupDefaultState {
    ...
    //registerDefaults: 定义一组默认的数据，从plist中取值时，如果没有值，则取这组默认数据，由于是单例 所以定义默认数据的方法只执行一次
}
...
- (NSInteger)tileSize {
    ...
    float SizeW = ([[UIScreen mainScreen] bounds].size.width - (self.dimension +1)* self.borderWidth - [self horizontalOffset]*2)/self.dimension;
    if (SizeW > 30) {
        return SizeW;
    }
    return 30;
...
- (NSInteger)horizontalOffset {
    return 15;
//  CGFloat width = self.dimension * self.tileSize + (self.dimension +1)* self.borderWidth;
